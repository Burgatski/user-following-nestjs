...
export class ArticleService {
  ...
  async findAll(
    ...
    const queryBilder = PostgresDataSource.getRepository(ArticleEntity)
    ...
    if (query.favorited) {
      ...
      if (ids.length > 0) {
        // проверяем наш articles.id на то что он находится в массиве залайканых ids
        queryBilder.andWhere('articles.id IN (:...ids)', { ids });
      } else {
        // обрываем queryBilder он вернет пустой массив потому что условие 1=0 ложное
    ...
    if (query.author) {
      ...
      queryBilder.andWhere('article.authorId = :id', {
        id: author.id,
      });
    }
